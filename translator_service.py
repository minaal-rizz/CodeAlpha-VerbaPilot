...
def challenge_pool() -> list[str]:
    data = _safe_load_json(BASE_DIR / "phrases.json")
    pool: list[str] = []
...
def _get_client():
    ...
    if not all([key, endpoint, region]):
        raise RuntimeError("Missing Azure Translator env vars.")
    return TextTranslationClient(endpoint=endpoint,
                                 credential=AzureKeyCredential(key),
                                 region=region)
...
@lru_cache(maxsize=1)
def _load_phrase_db() -> dict:
    idioms_raw = _load_first_existing(IDIOM_FILES)
    slangs_raw = _load_first_existing(SLANG_FILES)
    return {
        "idioms": _normalize(idioms_raw),
        "slangs": _normalize(slangs_raw)
    }
...
def explain_idioms_slang(text: str) -> list[dict]:
    ...
    db = _load_phrase_db()
    hits = []
    for typ, store in [("idiom", db["idioms"]), ("slang", db["slangs"])]:
        for phrase, meaning in store.items():
            if re.search(r"\b" + re.escape(phrase) + r"\b", txt):
                hits.append({"phrase": phrase, "meaning": meaning, "type": typ})
    return hits
...
@lru_cache(maxsize=1)
def challenge_pool() -> list[str]:
    raw = _load_first_existing(PHRASE_FILES)
    pool: list[str] = []
    if isinstance(raw, list):
        pool = [str(x).strip() for x in raw if str(x).strip()]
    elif isinstance(raw, dict):
        pool = [str(v).strip() for v in raw.values() if str(v).strip()]
    return pool if pool else DEFAULT_POOL
